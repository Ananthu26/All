...
const ALoginPage = () => {
  ...
  //   setError("");
  //   try {
  //     const response = await axios.post(`${API_URL}/login`, { email: username, password, userType: "a" });
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center", minHeight: "100vh", background: "linear-gradient(to top, #a1000b, #00246d)" }}>
        <div style={{ width: "400px", padding: "30px", borderRadius: "10px", background: "white", boxShadow: "0px 4px 10px rgba(0,0,0,0.2)" }}>
          <h2 style={{ textAlign: "center", marginBottom: "20px", color: "#00246d" }}> Admin Log In</h2>
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column" }}>
            ...
            <button type="submit" disabled={isLoading} style={{ width: "100%", padding: "12px", border: "none", borderRadius: "25px", background: "linear-gradient(to right, #a1000b, #00246d)", color: "white", fontSize: "16px", fontWeight: "bold", cursor: "pointer", transition: "all 0.3s ease" }}>{isLoading ? "LOGGING IN..." : "LOG IN"}</button>
          </form>
          <a href="/ALoginPage" style={{ color: "#777", textDecoration: "none", display: "block", textAlign: "center", marginTop: "15px" }}>Forgot password?</a>
          <a href="/ASignupPage" style={{ display: "block", textAlign: "center", padding: "10px 14px", border: "1px solid #a1000b", borderRadius: "25px", background: "transparent", color: "#a1000b", fontSize: "14px", cursor: "pointer", transition: "all 0.3s ease", textDecoration: "none", marginTop: "15px" }}>CREATE NEW</a>
        </div>
      </div>
    </>
  );
